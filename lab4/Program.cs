...
public struct TrainTrip
{
    ...
    public TrainData ToTrainData() => new()
    {
        ...
        SpeedByFormula = CalculatedSpeed,
        Label = averageSpeed,
    };
}
...
public class TrainData
{
    ...
    public float SpeedByFormula { get; set; }
...
internal static class Program
{
    ...
    private static void TrainModel(List<TrainTrip> trips)
    {
        ...
        IEstimator<ITransformer> pipeline = MlContext.Transforms
            .Concatenate(
                "Features",
                nameof(TrainData.Distance),
                nameof(TrainData.TravelTime),
                nameof(TrainData.SpeedByFormula))
        ...
        RegressionMetrics metrics = MlContext.Regression.Evaluate(predictions, labelColumnName: nameof(TrainData.Label));

        Console.WriteLine("Модель обучена.");
        Console.WriteLine($"Качество на текущих данных: R²={metrics.RSquared:F3}, MAE={metrics.MeanAbsoluteError:F2} км/ч");
    }
    ...
    private static void PredictSpeed()
    {
        ...
        float calculated = distance / travelTime;
        TrainData input = new() { Distance = distance, TravelTime = travelTime, SpeedByFormula = calculated };
        ...
        float predicted = Math.Max(0f, prediction.Score);
        ...
        Console.WriteLine($"Предсказанная средняя скорость: {predicted:F2} км/ч");
        Console.WriteLine($"Расчетная скорость distance / travelTime: {calculated:F2} км/ч");
        Console.WriteLine($"Разница: {Math.Abs(predicted - calculated):F2} км/ч");
    }
